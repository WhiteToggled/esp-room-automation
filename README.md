...
A FastAPI-based backend for managing device states via an **encrypted** SQLite database and MQTT.

## Features
- **Device Management**: Tracks device states (ON/OFF) in an encrypted SQLite database (SQLCipher).
...
## Prerequisites
- Python 3.11+
- SQLCipher system libraries (`libsqlcipher-dev` on Debian/Ubuntu)
- MQTT Broker (e.g., Mosquitto)

## Installation

1. Install system dependencies (for encryption):
   ```bash
...
- `DB_PASSPHRASE`: Passphrase for the encrypted SQLite database (default: `change-me-in-production`).
...
2. Run the container:
   ```bash
   docker run -p 8000:8000 -e MQTT_BROKER=your-broker-ip -e DB_PASSPHRASE=secret automation-server
   ```
