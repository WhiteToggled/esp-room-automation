...
\begin{itemize}[leftmargin=1.4em]
  \item is built against the fixed \code{BASE\_URL} (see \S\ref{sec:apiconfig});
...
\subsection{Server Configuration (\texttt{constants/apiConfig.ts})}
\label{sec:apiconfig}

The module exports two constants: \code{BASE\_URL}, fixed at
\code{https://nestboard.iotpro.uk}, and \code{TOKEN\_KEY}, the AsyncStorage key
under which the session token is held. Every request in \code{api/client.ts} and
\code{api/biometric.ts} is built against \code{BASE\_URL}.

The URL is not user-configurable: all installs talk to the same hosted
deployment, so pointing the app elsewhere means changing this constant and
rebuilding.
...
layout and animated entrance (\code{FadeInView}). \code{LoginScreen} also offers
biometric sign-in when the device has a key enrolled. Validation is performed
inline; network/credential errors are surfaced with specific messages (e.g.
invalid credentials vs.\ unreachable server, the latter including the target
URL).
...
The app ships pointed at the hosted backend
(\code{https://nestboard.iotpro.uk}); sign in with the admin account or a
provisioned user. To target a different deployment, change \code{BASE\_URL} in
\code{constants/apiConfig.ts} and rebuild.
